...
(function () {
  ...
  let DPR = getParticleDpr();
  const PARTICLE_ALPHA = 0.5;
  let W = 0, H = 0, dots = [];
  const mouse = { x: -9999, y: -9999 };

  function getParticleDpr() {
    const cap = isMobileViewport() || saveData ? 1.6 : 2;
  ...
  function spawnDots() {
    const density = isMobileViewport() ? 5600 : 4200;
    const maxDots = isMobileViewport() ? 320 : 700;
    const count = Math.min(Math.round((W * H) / density), maxDots);
  ...
  function burst(x, y) {
    ...
    const sparkTotal = isMobileViewport() ? 24 : 42;
  ...
  function shouldRunParticles() {
    return !document.hidden && !reducedMotion && root.dataset.perf !== "lite";
  }

  function startParticles() {
    if (!particlesFrame && shouldRunParticles()) {
      particlesFrame = requestAnimationFrame(tickParticles);
    }
  }
  ...
  document.querySelectorAll("[data-tilt]").forEach((card) => {
    card.addEventListener("pointermove", (e) => {
      if (!finePointer || reducedMotion || root.dataset.perf === "lite") return;
  ...
  function buildBriefPayload() {
    return {
      ...
      website: companyWebsiteInput ? companyWebsiteInput.value.trim() : "",
      page: location.href,
  ...
  form.addEventListener("submit", async (e) => {
    ...
    setSubmitting(true);
    try {
      const formEndpoint = String(window.REYRO_FORM_ENDPOINT || "/api/brief").trim();
